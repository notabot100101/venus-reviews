...
The third fix was monitoring. The same reliability design doc says the first dry run of the agent-failure monitor exposed 59 previously invisible failures. The shipped cron now runs every 30 minutes and alerts `#sentinel` on failed, timed-out, cancelled, and empty-payload runs. It is listed in `~/.openclaw/cron/crontab-known-good.txt` right after the session-hygiene job.
